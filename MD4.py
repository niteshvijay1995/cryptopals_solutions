...
class MD4:
	def __init__(self,a=0x67452301,b=0xefcdab89,c=0x98badcfe,d=0x10325476,length=None):
		self.A = a
		self.B = b
		self.C = c
		self.D = d
	...
	def update(self, message_string):
		msg_bytes = TXT_TO_BYTES(_pad(message_string,self.length))
	...
	def hexdigest(self):
		s = ''
		for h in [self.A,self.B,self.C,self.D]:
			s += (hex(h)[2:]).rjust(8, '0')
		return s
